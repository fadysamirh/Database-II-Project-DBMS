...
public class BPlusTree implements Serializable {
	...
	/**
	 * Super basic constructor, create everything using their default values...
	 *
	 * @throws IOException is thrown when we fail to open/create the binary tree
	 *                     file
	 */
	...
	/**
	 * Basic constructor that creates a B+ Tree instance based on an already
	...
	/**
	 *
	 * This constructor allows us to create a B+ tree as before given our
	 * configuration but allows us to tweak the I/O flags
	 *
	 *
	 * @param conf  B+ Tree configuration instance
	 ...
	 */
	@SuppressWarnings("unused")
	public BPlusTree(BPlusConfiguration conf, String mode, BPlusTreePerformanceCounter bPerf)
			throws IOException, InvalidBTreeStateException {
		this.conf = conf;
		this.bPerf = bPerf;
		initializeCommon();
	...
	/**
	 * This constructor allows the most customization as it allows us to set the I/O
	 * flags as well as the file name of the resulting file.
	 *
	 * @param conf         B+ Tree configuration instance
	 * @param mode         I/O mode
	 ...
	 */
	@SuppressWarnings("unused")
	public BPlusTree(BPlusConfiguration conf, String mode, String treeFilePath, BPlusTreePerformanceCounter bPerf)
			throws IOException, InvalidBTreeStateException {
		this.conf = conf;
		this.bPerf = bPerf;
		initializeCommon();
	...
	// Our New Constructor
	@SuppressWarnings("unused")
	public BPlusTree(String strTableName, String strColName) throws IOException, InvalidBTreeStateException {
	...
	/**
	 * Insert the key into the tree while also providing the flexibility of having
	 * unique keys or not at will.
	 *
	 * @param key    key to add
	 * @param value  value of the key
	 * @param unique allow duplicates for this run?
	 * @throws IOException                is thrown when any of the read/write ops
	 *                                    fail.
	 * @throws InvalidBTreeStateException is thrown when there is an inconsistency
	 *                                    in the tree blocks.
	 * @throws IllegalStateException      is thrown we have a null tree
	 * @throws NumberFormatException      is thrown when we have an invalid key
	 *                                    value (we only allow >= 0 as keys)
	 */
	@SuppressWarnings("unused")
	public void insertKey(long key, String value, boolean unique)
			throws IOException, InvalidBTreeStateException, IllegalStateException, NumberFormatException {
	...
	/**
	 *
	 * This function is based on the similar function prototype that is given by
	 * CLRS for B-Tree but is altered (quite a bit) to be able to be used for B+
	 * Trees.
	 *
	 * The main difference is that when the split happens *all* keys are preserved
	 * and the first key of the right node is moved up.
	 *
	 ...
	 * [ k3 ] / \ / \ / \ [ k1 k2 ] [ k3 k4 ]
	 *
	 * This function requires at least *3* page writes plus the commit of the
	 * updated page count to the file header. In the case that after splitting we
	 * have a new root we must commit the new root index to the file header as well;
	 * this happens transparently inside writeNode method and is not explicitly done
	 * here.
	 *
	 * @param n     internal node "parenting" the split
	 * @param index index in the node n that we need to add the median
	 */
	private void splitTreeNode(TreeInternalNode n, int index) throws IOException, InvalidBTreeStateException {
...
//                aChild.getNodeType().toString());
		...
		if (ynode.isInternalNode()) {
			TreeInternalNode zInternal, yInternal = (TreeInternalNode) ynode;

			zInternal = new TreeInternalNode(TreeNodeType.TREE_INTERNAL_NODE, generateFirstAvailablePageIndex(conf));
		...
		// we have a leaf
			...
			TreeLeaf zLeaf, yLeaf = (TreeLeaf) ynode, afterLeaf;

			zLeaf = new TreeLeaf(yLeaf.getNextPagePointer(), yLeaf.getPageIndex(), TreeNodeType.TREE_LEAF,
					generateFirstAvailablePageIndex(conf));
	 ...
	 *
	 * * Create an overflow page directly from a B+ TreeLeaf. * Add an overflow page
	 * directly after an existing one.
	 *
	...
	private void createOverflowPage(TreeNode n, int index, String value)
		...
		if (n.isOverflow()) {
			...
			novf = new TreeOverflow(-1L, ovf.getPageIndex(), generateFirstAvailablePageIndex(conf));
			...
			novf = new TreeOverflow(-1L, l.getPageIndex(), generateFirstAvailablePageIndex(conf));
...
//                    " for key: " + l.getKeyAt(index));
			...
			throw new InvalidBTreeStateException(
	...
	/**
	 * Binary search implementation for tree blocks; if not found returns the
	 * lower/upper bound position instead based on `rank`.
	 * 
	...
	/**
	 * This function is inspired from the one given in CLRS for inserting a key to a
	 * B-Tree but as splitTreeNode has been (heavily) modified in order to be used
	 * in our B+ Tree. It supports handling duplicate keys (if enabled) as well.
	 *
	 * It is able to insert the (Key, Value) pairs using only one pass through the
	 * tree.
	 *
	...
	private void insertNonFull(TreeNode n, long key, String value, boolean unique)
		...
		if (n.isLeaf()) {
			...
			int iadj = (n.getCurrentCapacity() > 0 && i == 0 && n.getFirstKey() > key) ? i : i - 1;
	 ...
	 * @throws IOException is thrown when an I/O operation fails
	 */
	private void parseOverflowPages(TreeLeaf l, int index, RangeResult res) throws IOException {
		TreeOverflow ovfPage = (TreeOverflow) readNode(l.getOverflowPointerAt(index));
		int icap = 0;
		while (icap < ovfPage.getCurrentCapacity()) {
			res.getQueryResult().add(new KeyValueWrapper(l.getKeyAt(index), ovfPage.getValueAt(icap)));
			icap++;
			// check if we have more pages
			if (icap == ovfPage.getCurrentCapacity() && ovfPage.getNextPagePointer() != -1L) {
	...
	public RangeResult rangeStartSearch(long minKey, boolean unique) throws IOException, InvalidBTreeStateException {
		...
		if (sMin.isFound()) {
			...
			int i = sMin.getIndex();
			while (sMin.getLeaf().getKeyAt(i) >= minKey) {
				rangeQueryResult.getQueryResult()
						.add(new KeyValueWrapper(sMin.getLeaf().getKeyAt(i), sMin.getLeaf().getValueAt(i)));
		...
		// given even if we did not find anything.
			...
			sMax = searchStartKey(minKey, unique);

			int i = sMax.getIndex();
			System.out.println(sMin.getLeaf().getKeyAt(i));
			while (sMin.getLeaf().getKeyAt(i) >= minKey) {
...
//				rangeQueryResult.getQueryResult()
//						.add(new KeyValueWrapper(sMax.getLeaf().getKeyAt(i), sMax.getLeaf().getValueAt(i)));
//
//				// check if we have an overflow page
...
//		}
	...
	public SearchResult searchStartKey(long key, boolean unique) throws IOException {
		return searchStartKey(this.root, key, false);
	}

	private SearchResult searchStartKey(TreeNode node, long key, boolean unique) throws IOException {
		// search for the key
		int i = binSearchBlock(node, key, Rank.PlusOne);
			...
			if (i >= 0 && i < node.getCurrentCapacity() && key >= node.getKeyAt(i)) {
				...
				// handle the case of duplicates where actual overflow pages exist
					...
					while (icap < ovfBuf.getCurrentCapacity()) {
						...
						if (icap == ovfBuf.getCurrentCapacity() && ovfBuf.getNextPagePointer() != -1L) {
	...
	public RangeResult rangeStopSearch(long maxKey, boolean unique) throws IOException, InvalidBTreeStateException {
		// SearchResult sMin = searchStopKey(maxKey, unique);
		SearchResult sMax;
		RangeResult rangeQueryResult = new RangeResult();
		// if (sMin.isFound()) {
		// read up until we find a key that's greater than maxKey
		...
		sMax = searchStopKey(maxKey, unique);

		int i = sMax.getIndex();
		// System.out.println(sMin.getLeaf().getKeyAt(i));
		while (sMax.getLeaf().getKeyAt(i) <= maxKey) {
			// System.out.println(i);
			rangeQueryResult.getQueryResult()
					.add(new KeyValueWrapper(sMax.getLeaf().getKeyAt(i), sMax.getLeaf().getValueAt(i)));
			...
			// check if we need to read the next block
			if (i == sMax.getLeaf().getCurrentCapacity()) {
				// check if we have a next node to load.
				if (sMax.getLeaf().getNextPagePointer() < 0)
				// if not just break the loop
				{
					break;
				}
				sMax.setLeaf((TreeLeaf) readNode(sMax.getLeaf().getNextPagePointer()));
				i = 0;
	...
	public SearchResult searchStopKey(long key, boolean unique) throws IOException, InvalidBTreeStateException {
		return searchStopKey(this.root, key, false);
	}

	private SearchResult searchStopKey(TreeNode node, long key, boolean unique)
			throws IOException, InvalidBTreeStateException {
		// search for the key
		// int i = binSearchBlock(node, key, Rank.PlusOne);
		int i = 0;
	...
	/**
	 * Handle range search queries with a bit of twist on how we handle duplicate
	 * keys.
	 *
	 ...
	 * duplicates they will be stored in one or multiple overflow pages depending on
	 * the number of duplicates. Hence we have to pay for those disk accesses as
	 * well.
	 *
	 ...
	 * @throws IOException                is thrown when an I/O operation fails
	 * @throws InvalidBTreeStateException is thrown when there are inconsistencies
	 *                                    in the blocks.
	 */
	@SuppressWarnings("unused")
	public SearchResult searchKey(long key, boolean unique) throws IOException, InvalidBTreeStateException {
		bPerf.incrementTotalSearches();
		return (searchKey(this.root, key, unique));
	}

	/**
	 * This function performs the actual search as described in searchKey
	 * description and is recursively called until we reach a leaf.
	 *
	...
	/**
	 * Function to delete a key from our tree... this function is again adopted from
	 * CLRS delete method but this was basically written from scratch and is loosely
	 * based on the actual notes.
	 *
	 * -- firstly it deletes the keys (all or first depending on unique flag) --
	 * secondly it updates the pool of empty pages -- thirdly performs merges if
	 * necessary (it capacity falls < degree-1) -- finally condenses the file size
	 * depending on load
	 *
	 ...
	 */
	@SuppressWarnings("unused")
	public DeleteResult deleteKey(long key, boolean unique) throws IOException, InvalidBTreeStateException {
		if (root.isEmpty()) {
	...
	/**
	 * That function does the job as described above; to perform easily the
	 * deletions we store *two* nodes instead on *one* in memory; the parent and the
	 * current. This also avoids adding information to the actual node file
	 * structure to account for the "parenting" link.
	 *
	 ...
	 *                                    in the blocks.
	 */
	public DeleteResult deleteKey(TreeNode current, TreeInternalNode parent, int parentPointerIndex, int parentKeyIndex,
			long key, boolean unique) throws IOException, InvalidBTreeStateException {

		// check if we need to consolidate
		if (current.isTimeToMerge(conf)) {
			// System.out.println("Parent needs merging (internal node)");
			TreeNode mres = mergeOrRedistributeTreeNodes(current, parent, parentPointerIndex, parentKeyIndex);
			if (mres != null) {
				current = mres;
			}
		}
	...
	/**
	 * Simple helper function to check if we can re-distribute the node values.
	 *
	 ...
	 *                                    in the blocks.
	 */
	private int canRedistribute(TreeNode with) throws InvalidBTreeStateException {
		if (with != null) {
			if (with.isInternalNode()) {
				if (isValidAfterRemoval(((TreeInternalNode) with), 1)) {
					return (1);
				}
			} else if (with.isLeaf()) {
				if (isValidAfterRemoval(((TreeLeaf) with), 1)) {
					return (1);
				}
			} else {
	 ...
	 * while updating the referring key of the parent node (always an internal
	 * node).
	 *
	 *
	 ...
	 * |--------| <----- |--------| | with | | to | |--------| -----> |--------|
	 *
	 * In this case we *remove* the *last* n elements from with and *push* them (in
	 * the order removed) into the destination node
	 *
	 * The parent key-pointer is updated with the first value of the receiving node.
	 *
	 * The other case is when we use the next pointer:
	 *
	 * |--------| <----- |--------| | to | | with | |--------| -----> |--------|
	 *
	 * In this case we *remove* the *first* n elements from with and add them *last*
	 * (in the order removed) into the destination node.
	 *
	 * The parent key-pointer is updated with the first value of the node we
	 ...
	 *                                    in the blocks.
	 */
	private void redistributeNodes(TreeLeaf to, TreeLeaf with, boolean left, TreeInternalNode parent,
			int parentKeyIndex) throws IOException, InvalidBTreeStateException {
		long key;
	 ...
	 *                                    in the blocks.
	 */
	private void redistributeNodes(TreeInternalNode to, TreeInternalNode with, boolean left, TreeInternalNode parent,
			int parentKeyIndex) throws IOException, InvalidBTreeStateException {
	...
	/**
	 * Function that merges two leaves together; in this case we *must* have two
	 * leaves, left and right that are merged and their parent that *must* be an
	 * internal node (or root).
	 *
	 * We also have the index of the parent that the pointers indicate these two
	 * leaves
	 *
	 ...
	 *
	 *
	 * The merge happens from right -> left thus the final result would be like
	 * this:
	 *
	 * parent ... | key | ... / \ | result | x
	 *
	 *
	 * So basically we dump the values of right to the left while updating the
	 * pointers.
	 *
	 *
	 *
	 ...
	 *                                    in the blocks.
	 */
	private TreeNode mergeNodes(TreeLeaf left, TreeLeaf right, TreeLeaf other, TreeInternalNode parent,
			int parentPointerIndex, int parentKeyIndex, boolean isLeftOfNext, boolean useNextPointer)
			throws IOException, InvalidBTreeStateException {

		if ((left.getCurrentCapacity() + right.getCurrentCapacity()) > conf.getMaxLeafNodeCapacity()) {
		...
		fixTheTopPointer(other, parent, parentPointerIndex, parentKeyIndex, isLeftOfNext, useNextPointer);
	...
	private void fixTheTopPointer(TreeNode other, TreeInternalNode parent, int parentPointerIndex, int parentKeyIndex,
	 ...
	 *                                    in the blocks.
	 */
	private void joinLeaves(TreeLeaf left, TreeLeaf right, int cap) throws InvalidBTreeStateException {
		for (int i = 0; i < cap; i++) {
	 ...
	 *                                    in the blocks.
	 */
	private void mergeNodes(TreeLeaf left, TreeLeaf right) throws IOException, InvalidBTreeStateException {
		right.setBeingDeleted(true);
		// join the two leaves together.
		int cap = right.getCurrentCapacity();
		joinLeaves(left, right, cap);
	...
	/**
	 * Function that merges two internal nodes together; in this case we *must* have
	 * two internal nodes, left and right that are merged and their parent that
	 * *must* be an internal node (or root).
	 *
	 * We also have the index of the parent that the pointers indicate these two
	 * internal nodes.
	 *
	 ...
	 *                                    in the blocks.
	 */
	private TreeNode mergeNodes(TreeInternalNode left, TreeInternalNode right, TreeInternalNode other,
			TreeInternalNode parent, int parentPointerIndex, int parentKeyIndex, boolean isLeftOfNext,
			boolean useNextPointer) throws IOException, InvalidBTreeStateException {

		// check if we can actually merge
		if ((left.getCurrentCapacity() + right.getCurrentCapacity()) > conf.getMaxInternalNodeCapacity()) {
	...
	/**
	 *
	 * This function handles the merging or redistribution of the nodes depending on
	 * their capacity; usually we just redistribute, if not we merge.
	 *
	 ...
	 *                                    in the blocks.
	 */
	public TreeNode mergeOrRedistributeTreeNodes(TreeNode mnode, TreeInternalNode parent, int parentPointerIndex,
			int parentKeyIndex) throws IOException, InvalidBTreeStateException {
	 ...
	 *                                    in the blocks.
	 */
	private TreeNode handleRootRedistributionOrMerging(TreeNode mnode) throws IOException, InvalidBTreeStateException {
		if (mnode.isInternalNode()) {
			...
			if (lChild.isLeaf()) {
				...
				TreeLeaf pLeaf = (TreeLeaf) lChild, nLeaf = (TreeLeaf) rChild;
				...
				/*
				 * else { System.out.println("-- Consolidating Root (internal -> internal)"); }
				 */

				TreeInternalNode rNode = (TreeInternalNode) mnode, lIntNode = (TreeInternalNode) lChild,
	 ...
	 *                                    in the blocks.
	 */
	private TreeNode handleLeafNodeRedistributionOrMerging(TreeNode mnode, TreeInternalNode parent,
			int parentPointerIndex, int parentKeyIndex) throws IOException, InvalidBTreeStateException {
		...
		if (nnum > 0 && npar) {
			// System.out.println("\t -- Redistributing split node with elements from
			// next");
		...
		// now check if we can redistribute with prev
			...
			if (nptr != null) {
				// System.out.println("\t -- Redistributing next node with elements from
				// split");
				if (splitNodeIsLeftChild) {
		...
		// we can't redistribute, try merging with next
			...
			mnode = mergeNodes(splitNode, nptr, pptr, parent, parentPointerIndex, parentKeyIndex, isLeftOfNext,
					/* useNextPointer = */ true);
		}
		// last chance, try merging with prev
			...
			mnode = mergeNodes(pptr, splitNode, nptr, parent, parentPointerIndex, parentKeyIndex, isLeftOfNext,
					/* useNextPointer = */ false);
		} else {
			throw new IllegalStateException(
					"Can't have both leaf " + "pointers null and not be root or no " + "common parent");
		}
	 ...
	 *                                    in the blocks.
	 */
	private void validateNeighbours(TreeLeaf prev, TreeLeaf split, TreeLeaf next) throws InvalidBTreeStateException {
		if (prev != null) {
	 ...
	 *                                    in the blocks.
	 */
	private TreeNode handleInternalNodeRedistributionOrMerging(TreeNode mnode, TreeInternalNode parent,
			int parentPointerIndex, int parentKeyIndex) throws IOException, InvalidBTreeStateException {
		...
		// can't redistribute; merging needs to happen, check which has t-1 elements
			...
			// now, check if we can merge with the left node
				...
				throw new InvalidBTreeStateException("Can't merge or " + "redistribute, corrupted file?");
			}
		}
	...
	/**
	 * Function that initially creates the tree. Here we always create a Leaf that
	 * acts as our Root, until we split it.
	 * 
	...
	private TreeNode createTree() throws IOException, InvalidBTreeStateException {
		if (root == null) {
			root = new TreeLeaf(-1, -1, TreeNodeType.TREE_ROOT_LEAF, generateFirstAvailablePageIndex(conf));
	...
	/**
	 * Map the short value to an actual node type enumeration value. This
	 * paradoxically is the opposite of that we do in the similarly named function
	 * in each node.
	 *
	 * @param pval a value read from the file indicating which type of node this is
	 * @return nodeType equivalent
	 * @throws InvalidPropertiesFormatException is thrown when the node is of an
	 *                                          unknown type
	 */
	private TreeNodeType getPageType(short pval) throws InvalidPropertiesFormatException {
		switch (pval) {
		...
		default: {
			throw new InvalidPropertiesFormatException("Unknown " + "node value read; file possibly corrupt?");
		}
		}
	}

	/**
	 * Calculate the page offset taking in account the for the lookup page at the
	 * start of the file.
	 *
	...
	/**
	 * Function that commits the allocation pool to the file; this can be done after
	 * each deletion or more unsafely before committing the file changes at the end.
	 *
	 * @throws IOException                is thrown when an I/O operation fails
	 * @throws InvalidBTreeStateException is thrown when there are inconsistencies
	 *                                    in the blocks.
	...
	private void commitLookupPage() throws IOException, InvalidBTreeStateException {
		...
		if (freeSlotPool.size() <= conf.getFirstLookupPageElements()) {
			System.out.println(" -- We only need a singular page for " + "overflow values"
			...
			for (i = 0; i < pages; i++) {
				// create the page
				lpOvf = createOverflowLookupPage(lookupPagesPool.get(i), lookupPagesPool.get(i + 1));

				// write it.
				for (int j = 0; j < cap && poolIndex < freeSlotPool.size(); j++, poolIndex++) {
			...
			System.out.println(" -- Multiple pages needed for the " + "overflow values" + "\n\tPages needed: " + pages
					+ "\n\tInitial page capacity: " + conf.getFirstLookupPageElements() + "\n\tPer Page capacity: "
					+ conf.getMaxLookupPageOverflowCapacity());
		}
	}

	/**
	 * This function adjust the file length by purging the high index pages that are
	 * used. Low index pages are purged last by design.
	 *
	 * @throws IOException is thrown when an I/O operation fails
	 */
	private void conditionFileLength() throws IOException {
		...
		System.out.println(
				"\n\n -- Conditioning file has been completed! " + "\n\tPurged pages: " + (purged - this.maxPageNumber)
						+ "\n\tNew file size: " + calculatePageOffset(this.maxPageNumber + 1) + " bytes");
	}
	...
	private boolean isInternalNode(TreeNodeType nt) {
		return (nt == TreeNodeType.TREE_INTERNAL_NODE || nt == TreeNodeType.TREE_ROOT_INTERNAL);
	}
	...
	public boolean isLeaf(TreeNodeType nt) {
		return (nt == TreeNodeType.TREE_LEAF || nt == TreeNodeType.TREE_LEAF_OVERFLOW
				|| nt == TreeNodeType.TREE_ROOT_LEAF);
	}

	/**
	 * Reads an existing file and generates a B+ configuration based on the stored
	 * values
	 *
	 ...
	 * @throws IOException is thrown when an I/O operation fails
	 */
	private void writeFileHeader(BPlusConfiguration conf) throws IOException {
		treeFile.seek(0L);
	...
	/**
	 * Opens a file descriptor to our B+ Tree storage file; it can handle already
	 * existing files as well without recreating them unless explicitly stated.
	 *
	...
	/**
	 * Just commit the tree by actually closing the FD thus flushing the buffers.
	 *
	 * @throws IOException                is thrown when an I/O operation fails
	 * @throws InvalidBTreeStateException is thrown when there are inconsistencies
	 *                                    in the blocks.
	...
	/**
	 * This function initializes the look-up page; in the simple case that it does
	 * not already exist it just creates an empty page by witting -1L all over it.
	 * If it exists then we load it into memory for further use.
	 *
	...
	private void initializeLookupPage(boolean exists) throws IOException {
		...
		// if we do, read it.
			...
			while (pindex != -1L) {
				...
				freeSlotPool.addAll(lpOvf.keyArray.stream().collect(Collectors.toList()));
				pindex = lpOvf.getNextPointer();
			}

			System.out.println("-- Parsed " + parsed + " lookup overflow pages and the initial one, totaling: "
...
//    @SuppressWarnings("unused")
	 ...
	 */
	private void deletePage(long pageIndex, boolean sort) throws IOException, InvalidBTreeStateException {
	 ...
	 *
	 * -- in case of length being greater than entry size, it is trimmed -- in case
	 * of length being less than entry size, it is appended with whitespaces.
	 *
	...
	private String conditionString(String s) {
		...
		if (s.length() > conf.getEntrySize()) {
			System.out.println("Satellite length can't exceed " + conf.getEntrySize() + " trimming...");
